function [W, CFL, error, h] = CompressibleNSTestWithPade(m, epsilon, t_end, x_end, k, c_p, c_V, kappa, rho_sol, mom_sol, E_sol, R1, R2, R3)
...
function [flux] = NS(vars, c_V, R, kappa, x, t)
    ...
    A2 = LHS1\(-RHS1*(mom.^2./rho + p));
    
    B2 = LHS2\(epsilon*4/3*RHS2*(mom./rho));

    C2 = (R2(x(1:end-1), t))';
    
    A3 = LHS1\(-RHS1*(E.*(mom./rho) + p.*(mom./rho)));
    
    B3 = LHS2\(epsilon*RHS2*(4/3*1/2*(mom./rho).^2 + kappa*T));
