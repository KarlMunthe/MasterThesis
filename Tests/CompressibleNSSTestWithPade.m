function [W, CFL, error, h] = CompressibleNSSTestWithPade(m, t_end, x_end, k, c_p, c_V, rho_sol, mom_sol, E_sol, R1, R2, R3)
...
function [flux] = NSS(vars, c_p, c_V, R, x, t)
    ...
    B1 = LHS2\(RHS1*(nu.*(RHS1*rho)));
    
    C1 = (R1(x(1:end-1), t))';
    
    A2 = LHS1\(-(RHS1*((mom.^2)./rho + p)));
    
    B2 = LHS2\(RHS1*(nu.*(RHS1*mom)));

    C2 = (R2(x(1:end-1), t))';
    
    A3 = LHS1\(-RHS1*(E.*(mom./rho) + p.*(mom./rho)));
    
    B3 = LHS2\(RHS1*(nu.*(RHS1*E)));
