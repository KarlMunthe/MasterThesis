function [Work, Heat] = Pade_NSS(m, x, t_end, k, mu, alpha, c_v, c_p, R, D0, PD1, PD2, rho)
%constants
...
function [w] = RK4(v, A, B, k)
    ...
    k1 = PD1\(D0*A) + PD2\(D0*(nu.*(D0*B)));
    k2 = PD1\(D0*(A + k*k1/2)) + PD2\(D0*(nu.*(D0*B + k*k1/2)));
    k3 = PD1\(D0*(A + k*k2/2)) + PD2\(D0*(nu.*(D0*B + k*k2/2)));
    k4 = PD1\(D0*(A + k*k3)) + PD2\(D0*(nu.*(D0*B + k*k3)));
